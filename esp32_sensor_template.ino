/*
 * ESP32 Multi-Value Sensor for Wind Tunnel Controller
 * 
 * This code broadcasts sensor announcements and sends multi-value data packets
 * to the Raspberry Pi Wind Tunnel Controller via UDP.
 * 
 * Features:
 * - WiFi connectivity with auto-reconnect
 * - UDP discovery announcements (broadcast on port 5555)
 * - Multi-value data transmission (configurable target)
 * - HTTP configuration API with persistent storage
 * - HX711 load cell support with automatic detection
 ...
 * - HX711 by Bogdan Necula (optional, for load cells)
 * 
 * Compatible with Raspberry Pi Wind Tunnel Controller v1.0+
 */
...
const bool MULTI_VALUE_MODE = true;  // true = multi-value (lift, drag, temp), false = single value

// Sensor Configuration
String sensorID = "esp32_sensor_1";     // Unique ID (will be saved to flash)
String sensorType = "force_balance";    // Type description
const char* firmwareVersion = "1.1.0";

// Network Configuration (will be set via HTTP API and saved to flash)
String targetIP = "";          // Raspberry Pi IP (configured by setup wizard)
int targetPort = 5000;         // UDP data port
...
float readLiftSensor() {
    // Example: Read from load cell, pressure sensor, etc.
    // Return value in Newtons
    static float phase = 0;
    phase += 0.1;
    return 10.0 + 5.0 * sin(phase); // Simulated sine wave
}

float readDragSensor() {
    // Example: Read from second sensor
    static float phase = 0;
    phase += 0.15;
    return 5.0 + 3.0 * cos(phase); // Simulated cosine wave
}
...
float readTemperature() {
    ...
    return constrain(temp, 15.0, 30.0);
}
...
void setupHTTPAPI() {
  ...
  server.on("/config", HTTP_POST, [](AsyncWebServerRequest *request) {}, NULL,
    [](AsyncWebServerRequest *request, uint8_t *data, size_t len, size_t index, size_t total) {
      ...
      if (sensorRate < 100) {
        errorMsg = "Invalid rate (must be >= 100ms)";
        configValid = false;
        sensorRate = oldRate;
      }
